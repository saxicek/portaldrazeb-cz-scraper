...
module.exports = function(baseUrl) {
    ...
    var search_query = {
        ...
        crawlAuction = function (doc) {
            if (!_.has(doc, 'doc.url')) {
                console.error('crawlAuction() must be invoked with object that has attributes "doc" and "doc.url"!');
                return Promise.reject();
            }
            var updated_doc = doc.doc;
            return rp(updated_doc.url)
                .then(function(response) {
                    updated_doc.raw_data = response;
                    updated_doc.fetched = true;
                    return response;
                })
                .then(parser.parseAuction)
                .then(function(data) {
                    updated_doc.data = data;
                    updated_doc.parsed = true;
                    return updated_doc;
                })
                .then(function(doc) {
                    console.log('Parsed ' + doc.url);
                    return doc;
                });
        },
        ...
        // function gets urls of all actions
